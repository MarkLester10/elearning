...
$faculties = $analytic->getFacultyAnalytics();
        ...
        <div class="my-6 bg-gray-100 p-4">
            <form class="flex flex-col space-y-4 md:space-y-0 md:flex-row items-center space-x-4" action="#" method="GET" id='filterform'>
                ...
                <div class="w-full">
                    ...
                    <a href="faculty_data.php" class="btn btn-warning btn-sm">Reset</a>
                </div>
            </form>
                ...
                <table class="table" id="resultTbl">
                    ...
                    <tbody>
                        <?php foreach ($faculties as  $faculty) : ?>
                            <tr>
                                <td>TCU-MCU - <?php echo $faculty->id ?></td>
                                <td><?php echo ucfirst($faculty->firstname) . ' ' . ucfirst($faculty->lastname) ?></td>
                                <?php if (isset($_GET['from_date']) && isset($_GET['to_date'])) : ?>
                                    <td class="font-weight-bold"><?php echo shortDate($_GET['from_date']) . ' - ' . shortDate($_GET['to_date']) ?></td>
                                <?php endif; ?>
                                <td>
                                    <?php if (isset($_GET['from_date']) && isset($_GET['to_date'])) : ?>
                                        <?php echo $analytic->getFacultiesDuration($faculty->id, $_GET['from_date'], $_GET['to_date']) ?>
                                    <?php else : ?>
                                        <?php echo $analytic->getFacultiesDuration($faculty->id) ?>
                                    <?php endif; ?>
                                </td>
